...
Combinando o número de mensagens (1, 2 ou 3 por semana), presença de interatividade (sim ou não), horário de entrega (tarde ou noite) e consistência do horário de entrega (sim ou nao)

criar uma árvore e pintar as folhas que são relevantes pra prova de conceito 
...
Transformamos tudo em semana (Por enquanto não)
...
- Importar a base (formato .dta) com os dados adminstrativos
...
Neste tópico, construimos o Feature Selection Space.

Temos features $f(i,r)$

$$ f(i,r)=F(m_{i, r \ \in \ t}) $$, onde $m_{i, r \ \in \ t}$ são as mensagens em tempo real dentro do período $t$: semana, quinzena, mês. A função $F()$ captura alguma feature das mensagens como: número de caracteres, presença da palavra "sim", caixa alta, etc

$$ I(i,r) = H(f_{i,r \ \in \ t}),$$ $r=tempo \ real$, onde $H()=(h_1(),h_2(),...,h_P())$ e $h_p: \digamma \to \Upsilon,\ p=(1,2,...,P)$ é a função que leva do conjunto $\digamma$, que contém as features das mensagens do indivíduo $i$ em $r_1,r_2,...,r_q \ \in \ t$, ao conjunto $\Upsilon$ de estatísticas das features para cada $i$ em $t$ . A função $h_p()$ pode ser: média, mínimo, máximo, mediana, varíancia.
...
```
Importante: a fundação h() transforma o vetor das features das mensagens num escalar
```
...
TODO: TROCAR A SOMA PELA VARIÂNCIA
...
COLOCAR A IMAGEM DO GRAFICO

# 3. Previsão de efeitos heterogêneos sobre dados reais oraculo
...
Não queremos que todos (100%) indivudos caiam na melhor amostra mas sim aumentar a propabibilidade 
...
- $$\Delta_{proxy} = mean_i(R_{proxy}_{i}}^{MAX} - R_proxy_i)$$

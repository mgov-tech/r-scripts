...
```
No momento, estamos estimando um modelo só, pro bimestre mas é importante desenvolver, como próximos passos, devemos estimar um modelo para cada semana.
```
...
Na próxima seção iremos aplicar um algorimo (Lasso) oara entender se as variações em características dessas mensagens são boas previsoras de variações em efeito em nota (i.e., se delta proxy consegue prever $\Delta Y{}$).

# 2. Proxy

```
Fundamental na análises é olhar deltas, e não níveis
```
...
Para podermos realocar aos alunos em novas variações, usamos o vetor de probabilidade com K entradas em que cada entrada k é:
...
Usamos as probabilidades geradas acima para realocar os participantes para uma nova variação a cada semana. À partir disso, recalculamos qual seria o impacto gerado e, usando as fórmulas acima, podemos estimar qual seria o ganho de ter realocado os alunos usando as previsões geradas pela proxy, pelos efeitos previstos $\hat{\gamma}_{k,i}$ e pela probabilidade de realocação $P_{i,k,B+1}$. 

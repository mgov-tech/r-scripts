...
Ao revisitar o histórico dos projetos de comunicação e engajamento voltado para a mudança de comportamento e impacto social constatamos que boa parte do impacto real possível não chega aos beneficiários pelo simples fato de existir uma limitaçao na entrega do melhor conteúdo, dada as informações disponíveis e adaptação a novas situações. 

O gráfico abaixo é retirado de um projeto real, em que cada participante foi sorteado para receber 1 de 24 variações possíveis de sequências de mensagens. 
...
A partir disso, foi possível identificar qual das 24 sequências geraria o impacto máximo para cada participante e verificar se o participante recebeu a sequência de máximo impacto. Por exemplo, o impacto máximo para o participante $i$ poderia ter sido alcançado se ele tivesse recebido a variação 20, mas ele recebeu na realidade a variação 6. (edited)

O resultado dessa análise está no histograma abaixo, que mostra a proporção de participantes que efetivamente recebeu a sua sequência de máximo impacto. Por exemplo, entre os participantes que receberam a sequência 13, ela também era a sequência de máximo impacto para 30% deles. Isto significa que  os outros 70% poderiam ter seu impacto maximizado se tivessem recebido outra sequência.

![](fig/v_max_h.png) 

# Prova de conceito

Nas próximas seções detalharemos os primeiros passos na construção da prova de conceito onde endereçaremos este problema por meio de uma abordagem com Inteligência Artificial. O objetivo em questão é ter um indicador de alta frequencia (proxy) para prever variação no impacto a partir das caracteristicas específicas nas interações recebidas pelos participantes do Eduq+

# Introdução

Combinando número de mensagens (1, 2 ou 3 por semana), presença de interatividade (sim ou não), horário de entrega (tarde ou noite) e consistência do horário de entrega (sim ou nao)
...
- As notas atualizadas por bimestre (neste caso são 3o e 4o bimestre)
- As mensagens de interação chegam numa frequência semanal 


```
Importante: devemos nos próximos passos, transformamos tudo em semana (Por enquanto não)
```
...
Rodar k regression trees: $$ \hat{\beta}_{k,i} = w_k(C_{i})$$, em que $C_{i}$ sao características da familia e do aluno i preencher $\hat{\beta}_{k,i}$ (k colunas para cada individuo, que seguem as k arvores)
...
$$OLS: Y^{LASSO}_{i,B} = \alpha + \sum_k \gamma_{k} * v_{k} + \epsilon_{i,B} $$
Rodar k regression trees: $$\hat{\gamma}_{k,i} = z(C_{i})$$ preencher $\hat{\gamma}_{k,i}$ (k colunas para cada individuo, que seguem as k arvores) 
...
Para podermos realocar as alunas em novas variações, usamos o vetor de probabilidade com K entradas em que cada entrada k é:
...
Computar: 
 ...
 $$R_{real_{i}}^{MAX} = max_k(\hat{\beta}_{k,i})$$ (isto é, o retorno máximo previsto para o aluno i)
 $$R_{proxy_i} = \hat{\gamma}_i$$ (isto é, o retorno previsto pela variação que o aluno i de fato recebeu)
 $$R_{proxy_{i}}^{MAX} = max_k(\hat{\gamma}_i)$$ (isto é, o retorno máximo previsto para o aluno i)
Computar também:
...
"A média do méximo que poderia ser ganho de impacto, comparando com o impacto previsto que cada teria se recebesse a variação com a máxima média"

 $$\Delta_{AI}^{ign} = mean_{i}(R_{proxy_{i}}^{MAX} - R_{real_i})$$
"A média do que poderia ser ganho de impacto usando proxies e randomizer, comparando com o impacto previsto do que cada um estava recebendo na real"
...
Usamos as probabilidades geradas acima para realocar as alunas para uma nova variação a cada semana. À partir disso, recalculamos qual seria o impacto gerado e, usando as formulas acima, podemos estimar qual seria o ganho de ter realocado os alunos
 
 $$\Delta_{proxy} = mean_{i}(R_{proxy_{i}}^{MAX} - R_{proxy_{i}})$$

 $$QUALITY^{OLS} = \Delta_{AI}^{OLS}/\Delta_{oracle}^{OLS}$$ (% do ganho de impacto que poderia ser obtido pela proxy)

# 7. How well does AI approximate the potential gains?

(impactcom.shinyapps.io/dashboard/)[impactcom.shinyapps.io/dashboard/]
